...
import DndContainer from "@/1Config/Zustand/DndContainer/DndContainer";
import DndItem from "@/1Config/Zustand/DndItems/DndItem";
...
const ItemsTest: ItemsTestComponentType = (props) => {
  ...
  return (
    <div className={clsx(s.itemsTestWrapper, className)}>
      <DndContainer items={items} sharedId="sharedTestId">
        {elems.map((item) => (
          <DndItem card={item} key={item.id}>
